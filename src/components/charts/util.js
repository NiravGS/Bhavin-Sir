...
export const transformPercentageTooltip = (options) => {
  return {
    ...options.tooltips,
    mode: 'label',
    callbacks: {
      label: function (tooltipItem, dataItem) {
        return parseFloat(dataItem.datasets[0].data[tooltipItem.index]).toFixed(2) + '%'
      }
    }
  }
}
