...
class Post extends Component {
  ...
  renderContent () {
    if (this.props.data.type === 'image') {
      ...
      return (
        <VideoPlayer
          autoplay={false} controls className='video-js card-img video-content mb-3' poster={this.props.data.image}
          sources={[{
          ...
          <InputGroup className='comment-contaiener'>
            ...
            <InputGroupAddon addonType='append'>
              <Button color='primary'>
                <span className='d-inline-block'>{messages['pages.send']}</span> <i className='simple-icon-arrow-right ml-2' />
              </Button>
            </InputGroupAddon>
          </InputGroup>
