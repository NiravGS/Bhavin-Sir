...
export default class GlideComponentThumbs extends React.Component {
  ...
  thumbsResize () {
    const perView = Math.min(this.props.settingsThumbs.perView, this.props.settingsImages.data.length)
  ...
  renderDots () {
    ...
    for (let i = 0; i < this.state.total; i++) {
      dots.push(
        <button className='glide__bullet slider-dot' key={i} data-glide-dir={'=' + i} />
      )
    }
    return dots
  }
